import { ARPAbet2PhoneticAlphabet } from './arpabet2phoneticalphabet';

describe('ARPAbet2PhoneticAlphabet', () => {
    const convertor = new ARPAbet2PhoneticAlphabet();

    it('Monosyllable', () => {
        // 单音节
        const result = convertor.toAmericanPhoneticAlphabet('F AW1 N D');
        expect(result).toEqual('faʊnd');
    });

    it('Double syllable', () => {
        // ri'trai  双音节
        const result = convertor.toAmericanPhoneticAlphabet('R IY0 T R AY1');
        expect(result).toEqual('riˈtraɪ');
    });

    it('Trisyllable', () => {
        // ˈɛniˌwʌn, -wən 三音节
        const result = convertor.toAmericanPhoneticAlphabet('EH1 N IY0 W AH2 N');
        expect(result).toEqual('ˈɛniˌwʌn');
    });

    it('Monosyllable', () => {
        // kʊd 单音节
        const result = convertor.toAmericanPhoneticAlphabet('K UH1 D');
        expect(result).toEqual('kʊd');
    });

    it('Double syllable', () => {
        // 双音节
        const result = convertor.toAmericanPhoneticAlphabet("W IY1 L K IY0 N S N")
    ...
    it('Edge test: wrong accent mark', () => {
        // 边缘测试：重音标识不对
        expect(() => {
            convertor.toAmericanPhoneticAlphabet("W IY1 L K IY4 N S N");
    ...
    it('Edge test: ARPAbet is wrong', () => {
        // 边缘测试：ARPAbet不对
        expect(() => {
            convertor.toAmericanPhoneticAlphabet("W IY1 L K IG N S N");
    ...
    it('Edge test: wrong accent mark position', () => {
        // 边缘测试：重音标识位置不对
        expect(() => {
            convertor.toAmericanPhoneticAlphabet("W IY1 L K IY N1 S N");
    ...
    it('Edge test: ARPAbet marked with accent is wrong', () => {
         // 边缘测试：标记重音的ARPAbet不对
        expect(() => {
            convertor.toAmericanPhoneticAlphabet("W IY1 L K IY X1 S N");
