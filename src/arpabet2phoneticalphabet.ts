...
export class ARPAbet2PhoneticAlphabet {
    private arpabetTree: Record<string, Phoneme>;
    ...
    private toWord(arpabet: string): Word | null {
        if (!arpabet) {
            return null;
        }
        let before: string;
        ...
        for (const arphabetPhoneme of arpabetPhonemes) {
            phoneme = this.arpabetTree[arphabetPhoneme];
            if (phoneme) {
                ...
                stress = this.stressDict[lastNum];
                before = arphabetPhoneme.slice(0, -1);
                phoneme = this.arpabetTree[before];
                if (phoneme) {
                    syllable.addPhoneme(phoneme);
                    if (stress) {
    ...
    toIPA(arpabet:string): string | null {
        // 转换成国际音标
        const word = this.toWord(arpabet);
        if (!word) {
            return null;
        }
        return word.toIPA();
    }
    toAmericanPhoneticAlphabet(arpabet:string): string | null {
        // 转换成美音
        const word = this.toWord(arpabet);
        if (!word) {
            return null;
    ...
    toEnglishPhoneticAlphabet(arpabet:string): string | null {
        /// 转换成英音
        const word = this.toWord(arpabet);
        if (!word) {
            return null;
